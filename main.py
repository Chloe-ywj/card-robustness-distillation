...
def train():
    ...
    BEST_CLEAN_MODEL_PATH = "./distbert_test_results/A0.5B0.5/best_clean_student_model"
    BEST_ROBUST_MODEL_PATH = "./distbert_test_results/A0.5B0.5/best_robust_student_model"
    ...
    for epoch in range(config.EPOCHS):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch + 1}/{config.EPOCHS}"):
            ...
            student_rep_clean = student_outputs_clean.hidden_states[-1][:, 0, :]

            student_outputs_adv = student_model(inputs_embeds=perturbed_embeddings,
                                                attention_mask=inputs['attention_mask'],
                                                output_hidden_states=True)
            student_rep_adv = student_outputs_adv.hidden_states[-1][:, 0, :]
            ...
            loss_rep = loss_functions.representation_transfer_loss(student_rep_clean, student_rep_adv)
            loss = config.GAMMA*loss_ce + config.ALPHA * (loss_kd_clean + 2 * loss_kd_adv) / 2 + config.BETA * loss_rep
